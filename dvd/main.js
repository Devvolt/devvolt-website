...
*/
  ...
  var $elm$browser$Browser$Dom$getViewport =
    _Browser_withWindow(_Browser_getViewport);
  ...
  var $author$project$Bouncingdvdlogo$init = function (_v0) {
    return _Utils_Tuple2(
      ...
      A2(
        $elm$core$Task$perform,
        $author$project$Bouncingdvdlogo$UpdateSizeFromViewport,
  ...
  var $author$project$Bouncingdvdlogo$UpdateSize = F2(function (a, b) {
    return { $: 1, a: a, b: b };
  });
  ...
  var $elm$time$Time$onEffects = F3(function (router, subs, _v0) {
    ...
    var _v1 = A6(
      ...
      newTaggers,
      processes,
      _Utils_Tuple3(_List_Nil, $elm$core$Dict$empty, $elm$core$Task$succeed(0))
    );
    var spawnList = _v1.a;
  ...
  var $elm$browser$Browser$Events$subscription =
    _Platform_leaf("Browser.Events");
    ...
    return $elm$core$Platform$Sub$batch(
      _List_fromArray([
        ...
        $elm$browser$Browser$Events$onResize(
          $author$project$Bouncingdvdlogo$UpdateSize
        ),
      ])
    );
  };
  ...
  var $author$project$Bouncingdvdlogo$updateImage = function (render) {
    var newIndex =
      _Utils_cmp(render.L + 1, $elm$core$Array$length(render.Z) - 1) > 0
        ? 0
        : render.L + 1;
    return _Utils_update(render, { L: newIndex });
  };
  var $author$project$Bouncingdvdlogo$updatePosition = F3(function (
    position,
    screen,
    speed
  ) {
    var newDirY = position.N
      ? !(_Utils_cmp(position.B, screen.j - position.j) > 0)
      : position.B - speed < 0;
    var newY = newDirY ? position.B + speed : position.B - speed;
    var newDirX = position.M
      ? !(_Utils_cmp(position.A, screen.n - position.n) > 0)
      : position.A - speed < 0;
    var newX = newDirX ? position.A + speed : position.A - speed;
    return _Utils_update(position, {
      M: newDirX,
      N: newDirY,
      A: newX,
      B: newY,
  ...
  var $author$project$Bouncingdvdlogo$updateView = function (model) {
    if (model.P) {
      var oldModel = model;
      var newPosition = A3(
        $author$project$Bouncingdvdlogo$updatePosition,
        model.m,
        model.H,
        model.an
      );
      var newRender =
        !_Utils_eq(oldModel.m.M, newPosition.M) ||
        !_Utils_eq(oldModel.m.N, newPosition.N)
          ? $author$project$Bouncingdvdlogo$updateImage(model.q)
          : model.q;
      return _Utils_update(model, { m: newPosition, q: newRender });
    } else {
      return model;
    }
  };
  var $author$project$Bouncingdvdlogo$update = F2(function (msg, model) {
    switch (msg.$) {
      case 0:
        return _Utils_Tuple2(
          _Utils_update(model, { P: !model.P }),
      ...
      default:
        ...
        return _Utils_Tuple2(
          $author$project$Bouncingdvdlogo$updateView(
            _Utils_update(model, { aa: newTime })
          ),
          $elm$core$Platform$Cmd$none
        );
    }
  ...
  var $author$project$Bouncingdvdlogo$view = function (model) {
    ...
    var time = $elm$core$String$fromInt($elm$time$Time$posixToMillis(model.aa));
    var image =
      model.q.ac +
      ("/" +
        A2(
      ...
      _List_fromArray([
        A2(
          ...
          _List_fromArray([
            A2(
              $elm$html$Html$img,
              _List_fromArray([$elm$html$Html$Attributes$src(image)]),
              _List_Nil
            ),
          ])
        ),
  ...
  var $author$project$Bouncingdvdlogo$main = $elm$browser$Browser$element({
    bh: $author$project$Bouncingdvdlogo$init,
    bu: $author$project$Bouncingdvdlogo$subscriptions,
    bx: $author$project$Bouncingdvdlogo$update,
    by: $author$project$Bouncingdvdlogo$view,
  });
  _Platform_export({
    Bouncingdvdlogo: {
      init: $author$project$Bouncingdvdlogo$main(
